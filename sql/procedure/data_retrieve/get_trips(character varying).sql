...
--This procedure filles the field trip_id of taxi.gps_raw of a given user
...
begin
	...
	for v_record in
	select
		T1.timestamp s, T2.timestamp e
	from
		T1 inner join T2 on
			T1.row_number = T2.row_number
	loop
		raise notice 'processing % b % e %', v_uid, v_record.s, v_record.e;
		update taxi.gps_raw 
		set 
			trip_id = v_id
		where
